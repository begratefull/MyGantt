...
class DashboardService:
    """
    A service class responsible for transforming raw schedule data into aggregated
    KPIs, analytics payloads, and charting structures for the dashboard views.
    """
    ...
    def parse_variance(val: Any) -> float:
        """
        Safely parses a variance string (e.g., '3 days', '-1 days') into a float.

        Args:
            val (Any): The raw variance value.

        Returns:
            float: The parsed numeric variance, or np.nan if invalid.
        """
    ...
    def _calc_business_days(df: pd.DataFrame, col1: str, col2: str) -> pd.Series:
        """
        Calculates the exclusive business day variance between two date columns.

        Args:
            df (pd.DataFrame): The source dataframe.
            col1 (str): The starting date column name.
            col2 (str): The ending date column name.
    ...
    def enrich_dataframe(df: pd.DataFrame) -> pd.DataFrame:
        """
        Injects additional calculated columns into the dataframe needed for analytics.

        Args:
            df (pd.DataFrame): The base schedule dataframe.
    ...
    def split_base_data(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Splits the enriched dataframe into active and completed subsets.

        Args:
            df (pd.DataFrame): The full enriched schedule dataframe.

        Returns:
            Tuple[pd.DataFrame, pd.DataFrame]: (active_df, completed_df)
        """
        if df.empty:
            return df, df
        ...
        try:
            ...
            if 'LINE_COUNT' not in enriched_df.columns:
                enriched_df['LINE_COUNT'] = 1
    ...
    def _get_current_backlog(active_df: pd.DataFrame) -> pd.DataFrame:
        """
        Returns the current active backlog.

        Args:
            active_df (pd.DataFrame): The active dataset.

        Returns:
            pd.DataFrame: The unmodified active dataset.
        """
        return active_df
    ...
    def _filter_completed_by_date(comp_df: pd.DataFrame, start_date: pd.Timestamp,
        ...
        """
        Filters the completed dataframe to include only items completed within a date range.

        Args:
            comp_df (pd.DataFrame): The completed items dataframe.
            start_date (pd.Timestamp): The inclusive start date.
            end_date (Optional[pd.Timestamp]): The inclusive end date.
    ...
    def _calc_subset_metrics(comp_df: pd.DataFrame) -> Dict[str, float]:
        """
        Calculates variance, productivity, and on-time delivery percentages for a subset.

        Args:
            comp_df (pd.DataFrame): The subset of completed items.
    ...
    def _apply_exact_mask(df: pd.DataFrame, col: str, exact_val: str) -> pd.DataFrame:
        """
        Filters a dataframe for an exact string match in a specific column.

        Args:
            df (pd.DataFrame): The source dataframe.
            col (str): The column to search.
            exact_val (str): The value to match.
    ...
    def _apply_regex_mask(df: pd.DataFrame, col: str, search_terms: str) -> pd.DataFrame:
        """
        Filters a dataframe using a regular expression search on a specific column.

        Args:
            df (pd.DataFrame): The source dataframe.
            col (str): The column to search.
            search_terms (str): The regex pattern to apply.
    ...
    def calculate_advanced_kpis(active_df: pd.DataFrame, comp_df: pd.DataFrame, full_df: pd.DataFrame,
        ...
        """
        Generates the complex KPI payload required for rendering the top dashboard cards,
        adjusting logic based on whether a specific team or the global view is selected.

        Args:
            active_df (pd.DataFrame): The active backlog dataframe.
            comp_df (pd.DataFrame): The completed items dataframe.
            full_df (pd.DataFrame): The complete schedule dataframe.
            current_team_filter (str): The currently active UI team filter.

        Returns:
            Dict[str, Any]: A nested dictionary payload containing the calculated KPIs.
        """
        default_kpis: Dict[str, Any] = {
            'global': {'backlog_total': 0, 'backlog_prod': 0, 'backlog_sub': 0, 'delivery_ytd': np.nan,
                       'delivery_cur': np.nan},
            'view_type': 'health',
            'cards': []
        }
        ...
        try:
            ...
            if 'REQUIREMENT' in curr_active_df.columns:
                is_prod_act = curr_active_df['REQUIREMENT'].str.contains(AppConstants.PROD_REQ_PATTERN, case=False,
                                                                         na=False, regex=True)
                is_sub_act = curr_active_df['REQUIREMENT'].str.contains(sub_regex, case=False, na=False, regex=True)
            else:
                is_prod_act = pd.Series(False, index=curr_active_df.index)
                is_sub_act = pd.Series(False, index=curr_active_df.index)
            ...
            if team_upper == "ALL TEAMS":
                ...
                for t_name in [AppConstants.CUSTOM_TEAM_LABEL, AppConstants.STANDARD_TEAM_LABEL]:
                    t_full = full_df[
                        full_df['CALC_TEAM'] == t_name] if 'CALC_TEAM' in full_df.columns else pd.DataFrame()
                    t_act = curr_active_df[curr_active_df[
                                               'CALC_TEAM'] == t_name] if 'CALC_TEAM' in curr_active_df.columns else pd.DataFrame()
                    t_comp = comp_cur[
                        comp_cur['CALC_TEAM'] == t_name] if 'CALC_TEAM' in comp_cur.columns else pd.DataFrame()

                    dates_in = pd.to_datetime(
                        t_full['DATE TO ENG'] if 'DATE TO ENG' in t_full.columns else pd.Series(dtype=str),
                        errors='coerce')
                ...
                for l_type in line_types:
                    ...
                    prod_ytd = DashboardService._apply_regex_mask(t_ytd, 'REQUIREMENT', AppConstants.PROD_REQ_PATTERN)
                    sub_ytd = DashboardService._apply_regex_mask(t_ytd, 'REQUIREMENT', sub_regex)

                    prod_cur = DashboardService._apply_regex_mask(t_cur, 'REQUIREMENT', AppConstants.PROD_REQ_PATTERN)
    ...
    def get_detailed_donut_data(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
        """
        Analyzes the active backlog to build a distribution profile of lines per engineer,
        including a breakdown of specific line requirements.

        Args:
            df (pd.DataFrame): The active backlog dataframe.

        Returns:
            Dict[str, Dict[str, Any]]: A dictionary payload structured for the pie charts.
        """
        distribution: Dict[str, Dict[str, Any]] = {}

        try:
            curr_active_df = DashboardService._get_current_backlog(df)
            if curr_active_df.empty or 'ASSIGNED TO' not in curr_active_df.columns:
                return distribution
            ...
            for eng in curr_active_df['ASSIGNED TO'].unique():
                ...
                total_lines = eng_df['LINE_COUNT'].sum() if 'LINE_COUNT' in eng_df.columns else len(eng_df)
                ...
                if 'REQUIREMENT' in eng_df.columns:
                    for req in eng_df['REQUIREMENT'].replace('', 'Uncategorized').unique():
                        ...
                        count = eng_df[eng_df['REQUIREMENT'].replace('', 'Uncategorized') == req_name][
                            'LINE_COUNT'].sum()
                        if count > 0:
    ...
    def get_dashboard_family_stats(df: pd.DataFrame) -> List[Dict[str, Any]]:
        """
        Aggregates production lines by fixture family to identify high-volume
        and high-value product lines.

        Args:
            df (pd.DataFrame): The schedule dataframe.

        Returns:
            List[Dict[str, Any]]: A list of dictionaries containing aggregated family stats.
        """
        if df.empty:
            return []

        try:
            is_prod = df['REQUIREMENT'].str.contains(AppConstants.PROD_REQ_PATTERN, case=False, na=False, regex=True)
            ...
            if fam_col not in prod_df.columns:
                return []
    ...
    def prepare_timeline_data(comp_df: pd.DataFrame, active_df: pd.DataFrame, start_date: pd.Timestamp) -> Tuple[
        ...
        try:
            ...
            if not h_df.empty:
                ...
                var_col = h_df['COMPLETION VARIANCE'] if 'COMPLETION VARIANCE' in h_df.columns else pd.Series(
                    dtype=float, index=h_df.index)
            ...
            f_df = active_df.copy()
            if not f_df.empty:
                est_col = f_df['EST END DATE'] if 'EST END DATE' in f_df.columns else pd.Series(dtype=str,
                                                                                                index=f_df.index)
    ...
    def get_engineer_performance(full_df: pd.DataFrame, engineer_name: str) -> Dict[str, Any]:
        """
        Isolates the data for a specific engineer to generate individual throughput,
        project volume, and radar chart KPIs.

        Args:
            full_df (pd.DataFrame): The complete schedule dataframe.
            engineer_name (str): The target engineer.

        Returns:
            Dict[str, Any]: The individual analytics payload.
        """
        default_payload = {
            'throughput': {'prod': {}, 'sub': {}},
            'projects': [],
        ...
        try:
            ...
            if 'ASSIGNED TO' not in full_df.columns:
            ...
            if 'REQUIREMENT' in eng_df.columns:
                is_prod = eng_df['REQUIREMENT'].str.contains(AppConstants.PROD_REQ_PATTERN, case=False, na=False,
                                                             regex=True)
                is_sub = eng_df['REQUIREMENT'].str.contains(r'APP|SUB|QUOT|QUOTE|APPROVAL|SUBMITTAL', case=False,
                                                            na=False, regex=True)
            else:
                is_prod = pd.Series(False, index=eng_df.index)
                is_sub = pd.Series(False, index=eng_df.index)
